...
/** Nested sections (legacy SimpleLanding-style prose); kept for this client component only. */
type LandingSection = {
  title: string;
  lead?: string[];
...
/**
 * Keyword SEO pages: DOM + classes match app/components/SimpleLandingPage.tsx
 * (hero, search block, genre strip, prose h2/h3/h4 flow, footer with social row).
 */
export default function SiteStyleKeywordLanding({ content }: Props) {
    ...
    try {
      ...
      if (result?.success && Array.isArray(result.data)) {
        const moviesData = result.data
          .filter((movie: MovieListItem) => movie.imdb_id && movie.imdb_id.trim() !== "")
  ...
  const articleBody = content.sections.flatMap((section, si) => {
    const nodes: React.ReactNode[] = [
      <h3 key={`s${si}-h3`} className="text-2xl font-bold text-gray-800 mt-12 mb-4">
        {section.title}
      </h3>,
    ];
    (section.lead ?? []).forEach((p, i) => {
      nodes.push(<p key={`s${si}-l${i}`}>{p}</p>);
    });
    const subs = section.subsections ?? [];
    subs.forEach((sub, ssi) => {
      const isFirstH4 = ssi === 0;
      const afterLead = (section.lead?.length ?? 0) > 0;
      const h4Mt = !isFirstH4 || afterLead ? "mt-8" : "";
      nodes.push(
        <h4
          key={`s${si}-ss${ssi}-h4`}
          className={`text-xl font-bold text-gray-800 mb-3 ${h4Mt}`}
        >
          {sub.title}
        </h4>
      );
      sub.paragraphs.forEach((p, pi) => {
        nodes.push(<p key={`s${si}-ss${ssi}-p${pi}`}>{p}</p>);
      });
    });
    if (!subs.length && section.paragraphs?.length) {
      section.paragraphs.forEach((p, pi) => {
        nodes.push(<p key={`s${si}-fp${pi}`}>{p}</p>);
      });
    }
    return nodes;
  });
      ...
      <div className="min-h-screen bg-[#f2f2f2]">
        <div className="max-w-4xl mx-auto px-6 py-12">
          <div className="text-center mb-16">
            ...
            <Link
              href="/home"
              className="inline-block bg-[#3fae2a] hover:bg-[#35a024] text-white px-8 py-3 rounded font-semibold transition-colors"
            >
          ...
          <div className="mb-12">
            <p className="text-gray-700 text-lg leading-relaxed">
              ...
              Fantasy, Game show, History, Horror, Musical, Mystery, News, Reality, Romance, Sci-Fi,
              TV Movie, Thriller, War
            </p>
          </div>

          <div className="prose prose-lg max-w-none">
            <h2 className="text-3xl font-bold text-gray-800 mb-6">{content.heading}</h2>
            ...
            <div className="space-y-6 text-gray-700 leading-relaxed">
              ...
              {articleBody.map((node, i) => (
                <Fragment key={`a-${i}`}>{node}</Fragment>
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              <div className="space-y-4">
                ...
                <p className="text-gray-400 text-sm">
                  Watch your favorite movies and TV shows online for free. High quality streaming
            ...
            <div className="border-t border-[#2b2b2b] mt-8 pt-8">
              <div className="text-center">
                <div className="text-gray-500 text-sm">Made with ❤️ for movie lovers</div>
              </div>
            </div>
          </div>
        </footer>
